     ...
     <div className="min-h-screen flex items-center justify-center p-6 bg-transparent font-sans">
       ...
       >
         <div className="text-center mb-8">
           <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Đăng ký</h2>
           <p className="text-sm text-gray-500 dark:text-gray-400">Bắt đầu hành trình cộng tác mới của bạn</p>
         </div>
         ...
         >
           <div className="grid grid-cols-2 gap-4">
             <Form.Item name="firstName" rules={[{ required: true, message: 'Nhập tên!' }]}>
               <Input prefix={<UserOutlined className="text-blue-500" />} placeholder="Tên" size="large" className="h-11 rounded-xl bg-white/50 dark:bg-black/50 border-white/20" />
             </Form.Item>
             <Form.Item name="lastName" rules={[{ required: true, message: 'Nhập họ!' }]}>
               <Input prefix={<UserOutlined className="text-blue-500" />} placeholder="Họ" size="large" className="h-11 rounded-xl bg-white/50 dark:bg-black/50 border-white/20" />
             </Form.Item>
           </div>
 
           <Form.Item name="email" rules={[{ required: true, type: 'email', message: 'Email không hợp lệ!' }]}>
             <Input prefix={<MailOutlined className="text-blue-500" />} placeholder="Email" size="large" className="h-11 rounded-xl bg-white/50 dark:bg-black/50 border-white/20" />
           </Form.Item>
 
           <Form.Item name="password" rules={[{ required: true, min: 6, message: 'Mật khẩu tối thiểu 6 ký tự!' }]}>
             <Input.Password prefix={<LockOutlined className="text-blue-500" />} placeholder="Mật khẩu" size="large" className="h-11 rounded-xl bg-white/50 dark:bg-black/50 border-white/20" />
           </Form.Item>
 
           <Form.Item name="confirmPassword" rules={[{ required: true, message: 'Xác nhận lại mật khẩu!' }]}>
             <Input.Password prefix={<LockOutlined className="text-blue-500" />} placeholder="Xác nhận mật khẩu" size="large" className="h-11 rounded-xl bg-white/50 dark:bg-black/50 border-white/20" />
           </Form.Item>
