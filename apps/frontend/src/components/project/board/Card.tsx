...
export function Card({ card, index }: CardProps) {
  ...
  return (
    <CardDraggable id={card.id} index={index}>
      <div
        className="group relative w-full rounded-lg bg-white dark:bg-gray-800 
        border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md 
        transition-all cursor-pointer p-3"
      >
        {/* Toggle check icon (ẩn khi chưa hover) */}
        <div
