...
export class CardService {
  ...
  async createCard(params: {
    ...
    this.logger.log(`Card created with ID: ${card.id}, emitting optimized event...`);
    this.eventEmitter.emit('card.created', {
      card: cardToReturn,
      columnId: card.columnId,
      projectId: card.projectId
    });
    this.logger.log(`Emitted card.created event for card ID: ${card.id}`);
  ...
  async removeCard(cardId: string) {
    ...
    this.logger.log(`Card deleted with ID: ${cardId}, emitting event...`);
    ...
    this.logger.log(`Emitted card.deleted and recycle.added events for card ID: ${cardId}`);
  ...
  async moveCard(params: {
    ...
    this.logger.log(`Emitted card.moved event for card ${cardId}`);
    return responseData;
  }
  ...
  async copyCard(params: {
    ...
    const newCard = await this.prisma.$transaction(async (tx: any) => {
      ...
      const created = await tx.card.create({
        data: {
          ...
          labels: {
            create: originalCard.labels.map((l: any) => ({ label: l.label })),
          },
        },
    ...
    this.logger.log(`Emitted card.copied event for new card ${newCard.id}`);
    return newCard;
  }
