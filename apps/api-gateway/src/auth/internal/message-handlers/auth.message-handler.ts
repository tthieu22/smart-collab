...
export class AuthMessageHandler {
  ...
  @MessagePattern({ cmd: 'auth.user.search' })
  async handleUserSearchInternal(@Payload() data: { q: string }) {
    try {
      const users = await this.userService.search(data.q);
  ...
  @MessagePattern({ cmd: 'auth.user.check-emails' })
  async handleCheckEmails(@Payload() data: { emails: string[] }) {
    try {
      const results = await this.userService.checkEmails(data.emails);
